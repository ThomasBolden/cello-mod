...

{
  ...
  // change kk, etc. to 'iz', 'ix'...
  for(int kk = 0; kk<kzp; kk++){
    for(int jj = 0; jj<kyp; jj++){
      for(int ii = 0; ii<kxp; ii++){

        srand((unsigned)gettime(NULL)+static_cast<size_t>(100*(kk+jj+ii)));

        int rnum = (rand() % max_blasts_) + 1; // should give number 1-max_blasts_
        rn_array[ii][jj][kk][0] = rnum;

        //if(half_empty_ && alternate%2){
        //  rnum = 0;
        //}
